...
export default function HomePage() {
  ...
  // 1) À la première charge de l'onglet, on lit sessionStorage pour "Bienvenue"
  useEffect(() => {
    if (typeof window === "undefined") return;
    const hasStarted = sessionStorage.getItem("lumaStarted");
    if (hasStarted === "true") {
      setStarted(true);
    }
  }, []);

  // 2) Quand on clique sur "C’est parti", on passe en mode présentation
  const handleStart = () => {
    sessionStorage.setItem("lumaStarted", "true");
    setStarted(true);
  };

  // 3) Tant que !started, on affiche la page de bienvenue
  if (!started) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-indigo-100 via-white to-indigo-200 text-center px-6">
        <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-3xl w-full space-y-8">
          <h1 className="text-5xl font-extrabold text-black">
            Bienvenue dans Luma
          </h1>
          <p className="text-gray-600 text-lg whitespace-nowrap overflow-hidden text-ellipsis">
            La première Intelligence Artificielle qui s'adapte à ta manière de penser.
          </p>
          <button
            className="px-8 py-4 text-lg font-semibold bg-indigo-600 text-white rounded-full shadow-lg hover:bg-indigo-700 transition transform hover:scale-105 duration-300"
            onClick={handleStart}
          >
            🚀 C’est parti
          </button>
  ...
  // 4) Une fois démarré, on affiche la présentation (début)
  return (
...
<>
  ...
  <h2 className="text-2xl font-semibold mt-10">✨ En une phrase :</h2>
      ...
      </ul>     {/* 6) Bouton d'inscription en bas */}
